...
Stage 4 runs only if the samplesheet's `bins_dir` column is populated for at least one sample (skip with `--skip_host_couple`). Stage 5 runs only if `--phabox_db` is set. Stage 6 runs only once the external DRAM-v tables exist (see below); it skips gracefully otherwise.
